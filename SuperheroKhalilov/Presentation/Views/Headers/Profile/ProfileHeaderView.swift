...
class ProfileHeaderView: UIView {
    ...
    @IBAction func cameraButtonAction(_ sender: UIButton) {
        delegate.didTapOnCameraButton(view: self, button: cameraButton, isDeletable: isDeletable)
    }
...
extension ProfileHeaderView: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        if let image = info[UIImagePickerController.InfoKey.originalImage] as? UIImage {
            delegate.didFinishPickingMediaWithInfo(cameraButton: cameraButton, profileImage: image, isDeletable: &isDeletable)
        }
